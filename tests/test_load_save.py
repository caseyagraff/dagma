...
def test_save(tmp_path):
    ...
    computed_value = n.compute(x=1013)
...
def test_manual_save_success(tmp_path):
    ...
    assert n2.value == computed_value
...
def test_bad_save_raises(tmp_path):
    ...
    with pytest.raises(Exception):
        ComputeNode(add_one, file_path=file_path, save=bad_save, deps=["x"]).compute(
            x=1
        )
